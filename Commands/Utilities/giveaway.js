...
module.exports = {
    name: 'giveaway',
    description: 'A basic giveaway command',
    category: 'Utilities',
    callback: ({ message, client }) => {
        ...
        async function giveaway() {
            ...
            var time2 = '';
            var time3 = '';
            if(!message.member.hasPermission('MANAGE_CHANNELS' || 'MANAGE_MESSAGES' || 'ADMINISTRATOR')) return message.channel.send('You don\'t have enough permission to execute this command.');
            if (message.content === `j!giveaway`) return message.channel.send(`You didn\'t state a duration or a prize for the giveaway.`)
            if (message.content !== `$j!giveaway`) {
                    ...
                    if (stated_duration_hours3 !== '0') {
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                if (!msg.reactions.cache.get('🎉').users.cache.size < 1) {
                                    const winner_embed = new Discord.MessageEmbed()
                                    .setTitle(`Prize for ${prize}`)
                                    .setColor('ORANGE')
                                    .setDescription(`Winner: ${winner}\nHosted by: ${message.author}`)
                                    .setTimestamp()
                                    .setFooter('Ended at')
                                    msg.edit(':tada: **GIVEAWAY ENDED** :tada:', winner_embed);
                                message.channel.send(`${winner} won the giveaway for ${prize}!`)
                                }
                            }, 1000);
